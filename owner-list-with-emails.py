...
with open('owner-list.csv', 'w', newline='') as csvfile:
	datawriter = csv.writer(csvfile,dialect='excel')
	datawriter.writerow(['SiteID','Title', 'URI', 'Public?', 'OwnerID', 'OwnerName', 'Email', 'Active?'])
...
# 		str(sites[x]['o:owner']['o:id']) + " from Omeka S API")
		...
		datawriter.writerow([sites[x]['o:id'], sites[x]['o:title'], basePublicUrl + sites[x]['o:slug'], 'Y' if sites[x]['o:is_public'] else 'N', ownerJSON['o:id'], ownerJSON['o:name'], ownerJSON['o:email'], 'Y' if ownerJSON['o:is_active'] else 'N'])
